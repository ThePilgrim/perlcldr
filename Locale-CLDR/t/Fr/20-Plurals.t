...
for (my $count = 0; $count < @results; $count++) {
	is ($locale->plural($count), $results[$count], "Plural for $count in fr_FR");
}

$locale = Locale::CLDR->new('fr_CA');
...
for (my $count = 0; $count < @results; $count++) {
	is ($locale->plural($count), $results[$count], "Plural for $count in fr_CA");
}
...
foreach my $start (qw(zero one two few many other)) {
	foreach my $end (qw(zero one two few many other)) {
		is ($locale->plural_range($start, $end), $results[$count++], "Plural range $start - $end in fr_CA");
	}
}
...
is ($locale->plural_range(2.73, 6.43), 'few', "Plural range 2.73 - 6.43 in fr_CA");