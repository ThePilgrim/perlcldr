...
# The version is generated from the API version, the integer part of the CLDR virsion and the revision number
# thus if a version of the code is generated against a point version of CLDR the revision number will need to be increased
...
# Likely sub-tags, used to generate defaults for missing parts of the Locale Identifier.
{
...
sub get_likely_script {
    ...
    # The likely subtag module is generated by this code so we can't use the with directly
    # Create the code via an eval
...
Please note that this code requires Perl 5.12.0 and above in the main. There are some parts that require
